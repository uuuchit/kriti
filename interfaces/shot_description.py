...
class ShotBriefDescription(BaseModel):
    ...
    narration_text: Optional[str] = Field(
        default=None,
        description="The exact, raw narration/voiceover text spoken during this shot. Plain text only — no speaker tags, no brackets, no quotes. This field is the sacred TTS input and must never be truncated.",
        examples=[
            "माता अंजनी ने वर्षों तक भगवान शिव की कठोर तपस्या की।",
            "The hero walked into the sunset, never looking back.",
            None,
        ],
    )
...
class ShotDescription(BaseModel):
    ...
    narration_text: Optional[str] = Field(
        default=None,
        description="The exact, raw narration text for TTS. Carried from ShotBriefDescription.",
    )
    # )
    # speaker: Optional[str] = Field(
    #     default=None,
