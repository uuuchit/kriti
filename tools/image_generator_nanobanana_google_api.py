...
class ImageGeneratorNanobananaGoogleAPI:
    ...
    @retry(stop=stop_after_attempt(5), after=after_func)
    async def generate_single_image(
        ...
        **kwargs,
        ...
        if not response.candidates or not response.candidates[0].content or not response.candidates[0].content.parts:
            ...
            logging.error(f"Image generation blocked (finish_reason={finish_reason}). Prompt: {prompt[:100]}...")
            raise ValueError(f"Image generation blocked by API (reason: {finish_reason})")
