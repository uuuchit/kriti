...
class Script2VideoPipeline:
    ...
    def __init__(
        ...
        self.aspect_ratio = getattr(video_generator, 'default_aspect_ratio', '16:9')
    ...
    async def __call__(
        ...
        if os.path.exists(final_video_path):
            ...
            video_clips = []
            for shot_description in shot_descriptions:
                vpath = os.path.join(self.working_dir, "shots", f"{shot_description.idx}", "video.mp4")
                if os.path.exists(vpath):
                    video_clips.append(VideoFileClip(vpath))
                else:
                    logging.warning(f"Shot {shot_description.idx} video missing, skipping.")
            if video_clips:
                final_video = concatenate_videoclips(video_clips)
                final_video.write_videofile(final_video_path, codec="libx264", preset="medium")
                print(f"☑️ Concatenated {len(video_clips)} videos, saved to {final_video_path}.")
            else:
    ...
    async def generate_frames_for_single_camera(
        ...
        """Generate first_frame for every shot in this camera. No last_frame, no transition video."""

        async def _gen_first_frame(shot_idx):
            ff_path = os.path.join(self.working_dir, "shots", f"{shot_idx}", "first_frame.png")
            if os.path.exists(ff_path):
                print(f"🚀 Skipped generating first_frame for shot {shot_idx}, already exists.")
                self.frame_events[shot_idx]["first_frame"].set()
                return

            print(f"🖼️ Generating first_frame for shot {shot_idx}...")
            available_image_path_and_text_pairs = []
            for character_idx in shot_descriptions[shot_idx].ff_vis_char_idxs:
                if character_idx >= len(characters):
                    continue
                identifier = characters[character_idx].identifier_in_scene
                if identifier in character_portraits_registry:
                    for view, item in character_portraits_registry[identifier].items():
                        available_image_path_and_text_pairs.append((item["path"], item["description"]))

            ff_selector_output_path = os.path.join(self.working_dir, "shots", f"{shot_idx}", "first_frame_selector_output.json")
            if os.path.exists(ff_selector_output_path):
                ...
                print(f"  🔍 Selecting references for shot {shot_idx}...")
                ff_selector_output = await self.reference_image_selector.select_reference_images_and_generate_prompt(
                    ...
                    frame_description=shot_descriptions[shot_idx].ff_desc
                )
                with open(ff_selector_output_path, 'w', encoding='utf-8') as f:
                    json.dump(ff_selector_output, f, ensure_ascii=False, indent=4)

            ref_pairs, prompt = ff_selector_output["reference_image_path_and_text_pairs"], ff_selector_output["text_prompt"]
            prefix = "".join(f"Image {i}: {text}\n" for i, (_, text) in enumerate(ref_pairs))
            full_prompt = f"{prefix}\n{prompt}"
            ref_paths = [p for p, _ in ref_pairs]

            try:
                ff_image = await self.image_generator.generate_single_image(
                    prompt=full_prompt, reference_image_paths=ref_paths,
                    aspect_ratio=getattr(self, 'aspect_ratio', '16:9'),
                )
            except Exception as e:
                if ref_paths:
                    logging.warning(f"Shot {shot_idx}: image gen failed with refs ({e}), retrying text-only...")
                    ff_image = await self.image_generator.generate_single_image(
                        prompt=prompt, reference_image_paths=[],
                        aspect_ratio=getattr(self, 'aspect_ratio', '16:9'),
                    )
                else:
                    raise

            ff_image.save(ff_path)
            self.frame_events[shot_idx]["first_frame"].set()
            print(f"  ☑️ first_frame for shot {shot_idx} saved.")

        # Generate all first_frames for this camera
        tasks = [_gen_first_frame(idx) for idx in camera.active_shot_idxs]
        await asyncio.gather(*tasks, return_exceptions=True)
        ...
        if os.path.exists(frame_image_path):
            ...
            try:
                ...
                if reference_image_paths:
                    logging.warning(f"Shot {shot_idx} {frame_type}: image gen failed with refs ({e}), retrying text-only...")
        ...
        return {
            character.identifier_in_scene: {
                "front": {
                    ...
                    "description": "A front view portrait of the character.",
                },
            }
        }
