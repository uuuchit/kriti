    ...
    async def generate_frames_for_single_camera(
        ...
        if os.path.exists(first_shot_ff_path):
            ...
            for character_idx in shot_descriptions[first_shot_idx].ff_vis_char_idxs:
                if character_idx >= len(characters):
                    logging.warning(f"Shot {first_shot_idx}: character index {character_idx} out of range (max {len(characters)-1}), skipping.")
                    continue
            ...
            if camera.parent_shot_idx is not None:
                ...
                if os.path.exists(transition_video_path):
                    ...
                    try:
                        ...
                        logging.warning(f"Transition video for shot {first_shot_idx} failed: {e}. Proceeding without it.")
                ...
                elif not os.path.exists(transition_video_path):
                    logging.warning(f"No transition video for shot {first_shot_idx}, skipping new camera image.")
                else:
        ...
        if shot_descriptions[first_shot_idx].variation_type in ["medium", "large"]:
            task = self.generate_frame_for_single_shot(
                ...
                visible_characters=[characters[idx] for idx in shot_descriptions[first_shot_idx].lf_vis_char_idxs if idx < len(characters)],
                character_portraits_registry=character_portraits_registry,
            )
        ...
        for shot_idx in camera.active_shot_idxs[1:]:
            first_frame_task = self.generate_frame_for_single_shot(
                    ...
                    visible_characters=[characters[idx] for idx in shot_descriptions[shot_idx].ff_vis_char_idxs if idx < len(characters)],
                    character_portraits_registry=character_portraits_registry,
                )
            ...
            if shot_descriptions[shot_idx].variation_type in ["medium", "large"]:
                last_frame_task = self.generate_frame_for_single_shot(
                    ...
                    visible_characters=[characters[idx] for idx in shot_descriptions[shot_idx].lf_vis_char_idxs if idx < len(characters)],
                    character_portraits_registry=character_portraits_registry,
                )
