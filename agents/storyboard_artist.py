...
- Split narration into contiguous phrases. No overlap. No gaps. The phrases must cover the full narration in order.
...
- Characters: Extracted from the narration (may be empty). Enclosed in <CHARACTERS> and </CHARACTERS>
- User requirement: e.g. shot count, style. Enclosed in <USER_REQUIREMENT> and </USER_REQUIREMENT>
- Style: Visual style. Enclosed in <STYLE> and </STYLE>
...
human_prompt_template_design_storyboard_from_narration = """
<NARRATION>
{narration_str}
</NARRATION>
...
class StoryboardArtist:
    ...
    @retry(stop=stop_after_attempt(3), after=after_func)
    async def design_storyboard_from_narration(
        self,
        narration: str,
        characters: List[CharacterInScene],
        user_requirement: Optional[str] = None,
        ...
        """Design storyboard from narration text. Each shot gets one narration phrase for sync."""

        class StoryboardResponse(BaseModel):
            storyboard: List[ShotBriefDescription] = Field(
                description="Storyboard with one shot per narration phrase. Each audio_desc must contain the exact phrase for that shot.",
            )

        narration_str = narration.strip()
        characters_str = (
            "\n".join([f"Character {i}: {c.identifier_in_scene}" for i, c in enumerate(characters)])
            if characters
            else "(none)"
        )
        user_requirement_str = (user_requirement or "").strip()
        style_str = (style or "Cinematic, photorealistic").strip()

        parser = PydanticOutputParser(pydantic_object=StoryboardResponse)
        messages = [
            (
                "system",
                system_prompt_template_design_storyboard_from_narration.format(
                    format_instructions=parser.get_format_instructions()
                ),
            ),
            (
                "human",
                human_prompt_template_design_storyboard_from_narration.format(
                    narration_str=narration_str,
    ...
    async def decompose_visual_description(
        ...
        characters_str = "\n".join([f"{char.identifier_in_scene}: (static) {char.static_features or ''}; (dynamic) {char.dynamic_features or ''}" for char in characters])
        ...
        return ShotDescription(
            ...
            narration_text=getattr(shot_brief_desc, 'narration_text', None),
        )
